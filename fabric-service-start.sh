...
serviceID=`docker run -d \
 ...
 -e "LOGS_PATH=/opt/service/data/service" \
 -p ${SERVICE_BIND_PORT}:${SERVICE_BIND_PORT} \
 --volume=${PROFILE_PATH}/hosts:/etc/hosts \
 --volume=${PROFILE_PATH}:/opt/profile \
 --volume=${SERVICE_NODE_HOME}/scripts:/opt/service/scripts \
 --volume=${SERVICE_NODE_HOME}/service-node/build/libs/service-node.jar:/opt/service/service-node/build/libs/service-node.jar \
 --volume=${SERVICE_NODE_HOME}/service-chain-code:/opt/service-chain-code \
 --volume=${PROFILE_PATH}/data/service:/opt/service/data/service \
...
grep -m 1 "ServiceNode\$ - Started" <( tail -f ${PROFILE_PATH}/data/service/stdout.log)
