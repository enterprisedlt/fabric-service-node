...
/**
  * @author Maxim Fedin
  */
object ProxyNode extends App {
    //
    private val Environment = System.getenv()
    private val LogLevel = Option(Environment.get("LOG_LEVEL")).filter(_.trim.nonEmpty).getOrElse("INFO")
    private val ProxyBindPort = Option(Environment.get("PROXY_SERVICE_BIND_PORT")).map(_.toInt).getOrElse(throw new Exception("Mandatory environment variable missing PROXY_SERVICE_BIND_PORT!"))
    //
    ...
    private val server = new Server(ProxyBindPort)
    ...
    server.setHandler(new JsonRestEndpoint(Util.createCodec, new ProxyRestEndpoint(fabricNetworkManager)))
    //
    setupShutdownHook()
    server.start()
    logger.info("Started.")
