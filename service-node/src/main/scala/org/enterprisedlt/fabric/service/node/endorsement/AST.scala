...
case class BFAOf(values: Iterable[ASTExpression]) extends CompositeExpression {
    override def reconstruct(values: Iterable[ASTExpression]): CompositeExpression = BFAOf(values)
}
